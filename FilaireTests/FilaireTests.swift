...
final class FilaireTests: XCTestCase {
    ...
    func testHostProfileTmuxDefaults() {
        ...
        XCTAssertEqual(profile.tmuxStartupCommand, "exec tmux new-session -A -s dev\n")
        ...
        XCTAssertEqual(profile.startupCommand, "exec tmux new-session -A -s dev\n")

        // Detach existing tmux sessions toggle (-D)
        var detachProfile = profile
        detachProfile.detachExistingTmux = true
        XCTAssertEqual(detachProfile.tmuxStartupCommand, "exec tmux new-session -A -D -s dev\n")
        XCTAssertEqual(detachProfile.startupCommand, "exec tmux new-session -A -D -s dev\n")
        ...
        XCTAssertEqual(customProfile.tmuxStartupCommand, "exec tmux new-session -A -s work-session\n")
        XCTAssertEqual(customProfile.startupCommand, "exec tmux new-session -A -s work-session\n")

        // Custom session with detach
        customProfile.detachExistingTmux = true
        XCTAssertEqual(customProfile.tmuxStartupCommand, "exec tmux new-session -A -D -s work-session\n")
        XCTAssertEqual(customProfile.startupCommand, "exec tmux new-session -A -D -s work-session\n")
        ...
        XCTAssertEqual(spacedProfile.tmuxStartupCommand, "exec tmux new-session -A -s 'my work session'\n")

        var quotedProfile = profile
        quotedProfile.customTmuxSession = "team's-app"
        XCTAssertEqual(quotedProfile.tmuxStartupCommand, "exec tmux new-session -A -s 'team'\\''s-app'\n")
    ...
    func testHardwareKeyboardInterceptionAndActions() {
        ...
        // 8. Cmd+Shift+D: Split Horizontally -> [0x02, '"']
    ...
    func testHostProfileTmuxSetClipboardToggle() throws {
        ...
        XCTAssertEqual(profile.tmuxStartupCommand, "tmux set -s set-clipboard on 2>/dev/null; exec tmux new-session -A -s dev\n")
        profile.detachExistingTmux = true
        XCTAssertEqual(profile.startupCommand, "tmux set -s set-clipboard on 2>/dev/null; exec tmux new-session -A -D -s dev\n")
